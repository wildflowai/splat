...
fn pack_y_direction(
    ...
    // Find Y range needed
    let min_y = cameras_in_x_range.iter().map(|p| p.y).fold(f64::INFINITY, f64::min);
    let max_y = cameras_in_x_range.iter().map(|p| p.y).fold(f64::NEG_INFINITY, f64::max);

    let min_bin = y_binner.bin_id(min_y);
    let max_bin = y_binner.bin_id(max_y);

    // Pre-filter cameras in buffered X range
    let buffered_cameras: Vec<Point2D> = all_cameras.iter()
    ...
    while current_bin <= max_bin {
        ...
        if best_end_bin.is_none() {
            ...
            if should_use_special_clustering(&y_groups, max_cameras) {
                return handle_clustering_edge_case(&y_groups, x_left, x_right, buffer_m, max_cameras);
            }
...
fn should_use_special_clustering(y_groups: &HashMap<i64, Vec<Point2D>>, max_cameras: usize) -> bool {
...
fn handle_clustering_edge_case(
    ...
    for group in y_groups.values() {
        if group.len() > max_cameras {
            ...
            let x_patches = partition_1d_horizontal(group, max_cameras, buffer_m, y_val, min_x, max_x);
...
fn partition_with_dp(
    ...
    let cameras: Vec<Point2D> = camera_positions.iter()
    ...
    let max_x = cameras.iter().map(|c| c.x).fold(f64::NEG_INFINITY, f64::max);
    let min_y = cameras.iter().map(|c| c.y).fold(f64::INFINITY, f64::min);
    let max_y = cameras.iter().map(|c| c.y).fold(f64::NEG_INFINITY, f64::max);
    ...
    if (min_x - max_x).abs() < f64::EPSILON {
        return Ok(partition_1d_vertical(&cameras, max_cameras, buffer_meters, min_x, min_y, max_y));
    }

    if (min_y - max_y).abs() < f64::EPSILON {
        return Ok(partition_1d_horizontal(&cameras, max_cameras, buffer_meters, min_y, min_x, max_x));
    }
    ...
    println!("✅ Created {} patches (aspect cost: {:.2})", 
