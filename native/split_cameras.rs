...
impl ColmapSplitter {
    ...
    pub fn split_cameras(&self) -> Result<SplitStats, Box<dyn std::error::Error>> {
        ...
        let results: Vec<Result<_, String>> = self.config.patches.par_iter().map(|patch| {
            ...
            let filtered_cameras: HashMap<u32, Camera> = cameras.iter()
        ...
        for &original_pt_id in sorted_point_ids {
            ...
            let mut new_track: Vec<TrackElement> = pt.track.iter()
                .filter_map(|el| {
                    if kept_images.contains_key(&el.image_id) {
                        idx_map.get(&el.image_id)?.get(&el.point2d_idx).map(|&new_idx| TrackElement {
            ...
            if !new_track.is_empty() {
                ...
                final_points3d.insert(new_point3d_id, Point3D { 
                    point3d_id: new_point3d_id, error: -1.0, track: new_track, ..pt.clone() 
                });
    ...
    fn update_images_with_new_point_ids(&self, images: &HashMap<u32, Image>, point_id_mapping: &HashMap<u64, u64>) -> HashMap<u32, Image> {
        images.iter().map(|(img_id, image)| {
            let updated_points2d = image.points2d.iter().map(|pt| Point2D {
                x: pt.x, y: pt.y,
                point3d_id: point_id_mapping.get(&pt.point3d_id).copied().unwrap_or(pt.point3d_id),
            }).collect();
            
            (*img_id, Image { points2d: updated_points2d, ..image.clone() })
        }).collect()
    }
...
pub fn split_cameras(config: CameraConfig) -> PyResult<HashMap<String, usize>> {
    let stats = ColmapSplitter::new(config).split_cameras()
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(format!("Camera splitting failed: {}", e)))?;
